...
import {
  ...
  upsertFinancialSnapshotImport,
  upsertMonthlyTrendRow,
...
/** Exposed for cutover tests — import commit uses Azure PG when URL is set. */
export function resolveImportCommitBackend() {
  return selectImportCommitBackend();
}
...
export async function commitImport(
  ...
  try {
    if (preview.templateType === "financial_snapshot") {
      ...
      await upsertFinancialSnapshotImport(organizationId, {
      ...
      for (let i = 0; i < validRows.length; i++) {
        ...
        await upsertMonthlyTrendRow(organizationId, {
      ...
      for (const row of validRows) {
        ...
        await upsertTransactionRow(organizationId, {
    ...
    await updateOrganizationDataSource(organizationId, "imported");
